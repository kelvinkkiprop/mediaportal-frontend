...
export class IndexComponent {
  ...
  index(){
    ...
    this.mProgress = true
    this.mLiveStreamService.allItems().subscribe({
      next: (response) => {
        if(response){
  ...
  onChangePage(item:any){
    // console.log(item)
    this.mProgress = true
    this.mLiveStreamService.paginateItems(item).subscribe({
      next: (response) => {
        if(response){
    ...
    swalWithBootstrapButtons.fire({
      ...
      // cancelButtonColor: '#3490dc'
          ...
          if (result.isConfirmed) {
            this.mProgress = true
            this.mLiveStreamService.deleteItem(item).subscribe({
              next: (response) => {
                if(response){
                  // console.log(response)
  ...
  onSearch(formValues: any){
    this.mProgress = true
    this.mLiveStreamService.searchItems(formValues).subscribe({
      next: (response) => {
        if(response){
          this.mItems = (response as any).data
          // console.log(response)
